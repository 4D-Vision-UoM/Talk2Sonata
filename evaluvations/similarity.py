...
OUTPUT_DIR = "demo_outputs/similarity"
CUSTOM_CONFIG = dict(
...
def get_point_cloud(coord, color=None, verbose=True):
    ...
    for i in range(len(coord)):
        ...
        coord_aligned = coord_.copy()
        coord_aligned[:, [1, 2]] = coord_aligned[:, [2, 1]]
        
        if color is not None:
            color_ = to_numpy(color[i])
        pcd = o3d.geometry.PointCloud()
        pcd.points = o3d.utility.Vector3dVector(coord_aligned)
...
def get_line_set(coord, line, color=(1.0, 0.0, 0.0), verbose=True):
    ...
    # Fix coordinate alignment - swap Y and Z to put floor on XZ plane
    coord_aligned = coord.copy()
    coord_aligned[:, [1, 2]] = coord_aligned[:, [2, 1]]
    
    line = to_numpy(line)
    colors = np.array([color for _ in range(len(line))])
    line_set = o3d.geometry.LineSet()
    line_set.points = o3d.utility.Vector3dVector(coord_aligned)
...
with torch.inference_mode():
    ...
    o3d.io.write_point_cloud(f"{OUTPUT_DIR}/similarity_global.ply", pcds[0])
    o3d.io.write_point_cloud(f"{OUTPUT_DIR}/similarity_local.ply", pcds[1])
    o3d.io.write_line_set(f"{OUTPUT_DIR}/similarity_line.ply", pcds[2])
